"""
Cuts FGs from KITTY BDD
does not work as there is no instance segm currently
"""
...
class Cutter(object):
  ...
  def __init__(self, path):
      self.pathlist = list(Path(path).glob('*.jpg'))
      self.im_path = str(self.pathlist[0])
      self.mask_path = self.im_path.replace("/images/", "/labels/").replace(".jpg", "_train_id.png")
      self.idx = 0
      self.instanceId_1 = 0

      self.im = Image.open(self.im_path)
      self.seg_map = Image.open(self.mask_path).convert('L')
      print(numpy.array(self.seg_map))
